  ...
  return html`
    <div class="card" style="width: 90%; margin: 10px;">
      <div class="card-body" style="width: 90%">
        ...
        <h6 class="card-title">
          Open a local TCP port and sends socket requests to a target hsync client and port
        </h6>
        ${updating
        ...
        ${error
          ...
          : ''}
        ${rpcResult ? html`<div class="alert alert-success" role="alert">${rpcResult}</div>` : ''}
        ${listeners?.length
          ? html`
        ...
        <div class="mb-3">
          <input
            type="number"
            class="form-control"
            placeholder="local listening port"
            onInput=${portInput}
            value=${port}
          />
          <input
        ...
        <div class="mdl-card__supporting-text">
          <button class="btn btn-primary" disabled=${updating} onClick=${addListener}>
            Add Listener
          </button>
        </div>
      </div>
